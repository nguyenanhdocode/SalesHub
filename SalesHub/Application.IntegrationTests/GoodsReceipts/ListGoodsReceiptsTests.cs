...
public class ListGoodsReceiptsTests : IClassFixture<ApplicationFixture>
    , IAsyncLifetime
{
    private readonly ApplicationFixture _fixture;
    private readonly List<Guid> _documentIds = new List<Guid>();
    int baseUnitId = 0, supplierId = 0, periodId1 = 0, productId1 = 0
    , productId2 = 0, branchId1 = 0, warehouseId1 = 0, warehouseId2 = 0
    , branchId2 = 0, periodId2 = 0;
    ...
    public async Task DisposeAsync()
    {
        ...
        foreach (var docId in _documentIds)
        {
            ...
            await dataRand.DeleteDocument(docId);
        }
    ...
    public async Task InitializeAsync()
    {
        ...
        _documentIds.Add(_command1Res.DocumentId);
        _documentIds.Add(_command2Res.DocumentId);
        _documentIds.Add(_command3Res.DocumentId);
        _documentIds.Add(_command4Res.DocumentId);
    }
    ...
    public async Task List_Should_Return_All()
    {
        ...
        int count = res.Rows.Where(p => _documentIds.Contains(p.DocumentId)).Count();
    ...
    public async Task Filter_By_DocumentNo_Should_Return_One()
    {
        ...
        var rows = res.Rows.Where(p => _documentIds.Contains(p.DocumentId));
    ...
    public async Task Paginate_Should_Success()
    {
        ...
        var res1 = await sender.Send(new ListGoodsReceiptsQuery { PageNum = 1, PageSize = 3, BranchIds = [branchId1, branchId2]}
        , CancellationToken.None);
        int count1 = res1.Rows.Count();

        Assert.Equal(3, count1);

        var res2 = await sender.Send(new ListGoodsReceiptsQuery { PageNum = 2, PageSize = 3, BranchIds = [branchId1, branchId2]}
        , CancellationToken.None);
    ...
    public async Task Paginate_With_Wrong_Number_Should_Success()
    {
        ...
        var res1 = await sender.Send(new ListGoodsReceiptsQuery { PageNum = 0, PageSize = 50, BranchIds = [branchId1, branchId2]}
        , CancellationToken.None);
        int count1 = res1.Rows.Count();
        ...
        var res2 = await sender.Send(new ListGoodsReceiptsQuery { PageNum = 1, PageSize = 0, BranchIds = [branchId1, branchId2]}
        , CancellationToken.None);
        int count2 = res2.Rows.Count();
        ...
        var res3 = await sender.Send(new ListGoodsReceiptsQuery { PageNum = 0, PageSize = 0}
        , CancellationToken.None);
